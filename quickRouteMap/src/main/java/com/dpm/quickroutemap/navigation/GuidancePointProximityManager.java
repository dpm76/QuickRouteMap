			...
			public void onGpsStatusChanged(int event) {
				switch (event){
					...
					case GpsStatus.GPS_EVENT_FIRST_FIX:
						Log.v(LOG_TAG, "GPS first fix.");
						_gpsFixed = true;
						setRouteGuidance(_guidanceProvider.getCurrentRouteGuidance());
						break;
					case GpsStatus.GPS_EVENT_STOPPED:
						Log.v(LOG_TAG, "GPS stopped");
						_gpsReady = false;
						removeRouteGuidance();
						break;
					default:
...
//	}
	...
	public void setRouteGuidance(GuidancePoint[] routeGuidance){
		...
		if(routeGuidance != null){
			Log.v(LOG_TAG, "Añadiendo guiado");
	...
	private void addGuidancePoint(GuidancePoint guidancePoint){
		...
		Log.v(LOG_TAG,"Añadiendo " + guidancePoint.getKey());
